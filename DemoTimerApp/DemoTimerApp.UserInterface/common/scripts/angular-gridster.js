...
/*global define:true*/
// � Siemens AG, 2017
//The angular-gridster.js file has been modified inorder to have a synchronization with the mashup layout grid
	    ...
	    return {
	        ...
	        link: function (scope, $el, attrs, gridster) {
	            ...
	            scope.previewStyle = function () {
	                ...
	                return {
	                    display: 'block',
	                    height: (gridster.movingItem.sizeY * Math.round(gridster.curRowHeight) - gridster.margins[0]) + 'px',
	                    width: (gridster.movingItem.sizeX * Math.round(gridster.curColWidth) - gridster.margins[1]) + 'px',
	                    top: (gridster.movingItem.row * Math.round(gridster.curRowHeight) + (gridster.outerMargin ? gridster.margins[0] : 0)) + 'px',
	                    left: (gridster.movingItem.col * Math.round(gridster.curColWidth) + (gridster.outerMargin ? gridster.margins[1] : 0)) + 'px'
	                };
	            };
	        }
	    };
		            ...
		            return function (scope, $elem, attrs, gridster) {
		                ...
		                function refresh(config) {
		                    ...
		                    if (gridster.colWidth === 'auto') {
		                        //Added Math.round to the following calculation
	...
	.controller('GridsterItemCtrl', function () {
	    ...
	    this.setElementPosition = function () {
	        if (this.gridster.isMobile) {
	            ...
	            //Added Math.floor for the following calculation
	            this.$element.css({
	                margin: 0,
	                top: (this.row * Math.floor(this.gridster.curRowHeight) + (this.gridster.outerMargin ? (this.gridster.margins[0]) : 0)) + 'px',
	                left: (this.col * Math.floor(this.gridster.curColWidth) + (this.gridster.outerMargin ? (this.gridster.margins[1]) : 0)) + 'px'
	            });
	        }
	    };
	    ...
	    this.setElementSizeX = function () {
	        if (this.gridster.isMobile) {
	            ...
	            //Added Math.floor for the following calculation
	            this.$element.css('width', (this.sizeX * Math.floor(this.gridster.curColWidth) - this.gridster.margins[1]) + 'px');
	        }
	    };
	    ...
	    this.getElementSizeX = function () {
	        //Added Math.floor for the following calculation
	        return Math.floor(this.sizeX * this.gridster.curColWidth - this.gridster.margins[1]);
	    };
	    ...
	    this.getElementSizeY = function () {
	        //Added Math.floor for the following calculation
	        return Math.floor(this.sizeY * this.gridster.curRowHeight - this.gridster.margins[0]);
	    };
		...
		function ($document, $window, GridsterTouch) {
		    function GridsterDraggable($el, scope, gridster, item, itemOptions) {
		        ...
		        function mouseMove(e) {
		            ...
		            elmX += diffX;
		            elmY += diffY;
		            //Added Math.round for the following calculation
		            $el.css({
		                'top': Math.round(elmY) + 'px',
		                'left': Math.round(elmX) + 'px'
		            });
	    ...
	    function GridsterResizable($el, scope, gridster, item, itemOptions) {
	        ...
	        function ResizeHandle(handleClass) {
	            ...
	            function mouseMove(e) {
	                ...
	                //Added Math.floor for the following calculation
	                $el.css({
	                    'top': Math.floor(elmY) + 'px',
	                    'left': Math.floor(elmX) + 'px',
	                    'width': Math.floor(elmW) + 'px',
	                    'height': Math.floor(elmH) + 'px'
	                });
