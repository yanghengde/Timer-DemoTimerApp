...
(function (window, $) {
    ...
    angular.module('ngGrid.services').factory('$domUtilityService', ['$utilityService', '$window', function ($utils, $window) {
        ...
        domUtilityService.getRealWidth = function (obj) {
            ...
            ngGridSwap(hiddenParents[0], props, function () {
        ...
        // © Siemens AG, 2014
        // since jquery2.2.0 has not exposed swap as a public method. So defined ngGridSwap to replace the call.
        function ngGridSwap(elem, options, callback, args) {
            var ret, name,
                old = {};
        ...
        domUtilityService.digest = function ($scope) {
            // © Siemens AG, 2018
            // to resolve $$phase of null issue
    ...
    // © Siemens AG, 2014
    // get the data items associated to this group and selection action to take.
    ngAggregate.prototype.getAggregateSelectArg = function (toggle) {
        if (toggle)
            this.entity.selected = !this.entity.selected;
        var arg = {
            entities: [],
            select: this.entity.selected,
        }
        angular.forEach(this.children, function (child, index) {
            arg.entities.push(child.entity);
        });
        return arg;
    };

    ngAggregate.prototype.getCheckState = function () {
    ...
    ngAggregate.prototype.showSvgIcon = function () {
        // © Siemens AG, 2018
    ...
    var ngEventProvider = function (grid, $scope, domUtilityService, $timeout) {
        ...
        self.setDraggables = function () {
            // © Siemens AG, 2015 - add check for grid.$root
        ...
        self.assignGridEventHandlers = function () {
            ...
            var windowResize = function () {
                ...
                windowThrottle = setTimeout(function () {
                    ...
                    // © Siemens AG, 2015
                    // to avoid occasional incorrect width when grid next to floated div
                    setTimeout(function () { domUtilityService.RebuildGrid($scope, grid); }, 100);
                }, 100);
            };
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
        ...
        self.getTemplate = function (key) {
            ...
            if (t && !TEMPLATE_REGEXP.test(t)) {
                // © Siemens AG, 2017
                //changed $http callback methods to be compatible with angular v1.6.3
                $http.get(t, {
                    cache: $templateCache
                })
        ...
        // © Siemens AG, 2014
        $scope.noDataMessage = self.config.noDataMessage;
        $scope.selectAggregate = function (arg) {
            if (self.config.selectAggregateCallback)
                self.config.selectAggregateCallback(arg.entities, arg.select);
            // © Siemens AG, 2017
            // to select headercheckbox
            var headerCheckbox;
            for (var i = 0 ; i < $scope.renderedRows.length; i++) {
                if ($scope.renderedRows[i].children) {
                    headerCheckbox = true;
                    if (!$scope.renderedRows[i].entity.selected) {
        ...
        $scope.adjustScrollTop = function (scrollTop, force) {
            ...
            // © Siemens AG, 2014
            var rowIndex = Math.floor(scrollTop / (self.config.rowHeight + 1));
            var newRange;
    ...
    var ngRow = function (entity, config, selectionProvider, rowIndex, $utils) {
        ...
        // © Siemens AG, 2014
        this.offsetTop = this.rowIndex * (config.rowHeight + 1);
    ...
    ngRow.prototype.toggleSelected = function (event) {
        ...
        // © Siemens AG, 2015 - do not support selecting by checkbox only. allow callback to fire so wrapper of ng-grid can handle it.
    ...
    ngRow.prototype.alternatingRowClass = function () {
        ...
        var classes = {
            'ngRow': true,
            // © Siemens AG, 2014 - support overriding row class
        ...
        // © Siemens AG, 2014
        classes[this.config.evenClass] = isEven;
        classes[this.config.oddClass] = !isEven;
        classes[this.config.selectedClass] = this.selected;
        return classes;
    };
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        var self = this;
        // © Siemens AG, 2014
        var customRowClasses = grid.config.customRowClasses || {};
        ...
        self.rowConfig = {
            ...
            // © Siemens AG, 2014 - support for overriding the row class
            oddClass: customRowClasses.odd || 'odd',
            evenClass: customRowClasses.even || 'even',
            selectedClass: grid.config.showRowHighlight ? (customRowClasses.selected || 'selected') : '',
        ...
        self.buildAggregateRow = function (aggEntity, rowIndex) {
            ...
            if (!agg) {
                // © Siemens AG, 2015
                // did it previously exist?  If so, use previous collapse state
                var collapsed = (self.prevAggCache[aggEntity.aggIndex] &&
                    self.prevAggCache[aggEntity.aggIndex].field === aggEntity.gField &&
                    self.prevAggCache[aggEntity.aggIndex].label === aggEntity.gLabel)
                ? self.prevAggCache[aggEntity.aggIndex].collapsed : grid.config.groupsCollapsedByDefault;
        ...
        self.renderedChange = function () {
            ...
            for (var i = self.renderedRange.topRow; i < self.renderedRange.bottomRow; i++) {
                if (temp[i]) {
                    // © Siemens AG, 2014
                    temp[i].offsetTop = i * (grid.config.rowHeight + 1);
                    rowArr.push(temp[i]);
                }
            }
        ...
        self.renderedChangeNoGroups = function () {
            ...
            for (var i = self.renderedRange.topRow; i < self.renderedRange.bottomRow; i++) {
                if (grid.filteredRows[i]) {
                    ...
                    // © Siemens AG, 2014
                    grid.filteredRows[i].offsetTop = i * (grid.config.rowHeight + 1);
        ...
        self.fixRowCache = function () {
            ...
            // © Siemens AG, 2017
            // to select group checkbox and headercheckbox
            var areRowsSelected;
            angular.forEach(grid.rowFactory.aggCache, function (aggCache) {
                areRowsSelected = true;
                for (var i = 0 ; i < aggCache.children.length; i++) {
                    if (!aggCache.children[i].entity.selected) {
                        areRowsSelected = false;
                        break;
                    }
                }
                aggCache.entity.selected = areRowsSelected;
            })            
        ...
        self.getGrouping = function (groups) {
            // © Siemens AG, 2015
            self.prevAggCache = self.aggCache;
            self.aggCache =[];
            self.numberOfAggregates = 0;
            self.groupedData = {};
            // Here we set the onmousedown event handler to the header container.
            ...
            // © Siemens AG, 2015
            // expand/collapse any groups that were re-created with non-default collapsed value
            angular.forEach(self.aggCache, function (agg) {
                if (agg.collapsed != grid.config.groupsCollapsedByDefault) {
                    agg.notifyChildren();
                }
            });
        };
    ...
    var ngSelectionProvider = function (grid, $scope, $parse, $utils) {
        ...
        // © Siemens AG, 2017
        // function to manage header checkbox selection
        self.updateCheckboxSelection = function () {
            var headerCheckbox;
            for (var i = 0 ; i < $scope.renderedRows.length; i++) {
                if ($scope.renderedRows[i].children) {
                    headerCheckbox = true;
                    ...
                    headerCheckbox = true;
                    if (!$scope.renderedRows[i].selected) {
                        headerCheckbox = false;
                        break;
                    }
        ...
        self.setSelection = function (rowItem, isSelected) {
            ...
            // © Siemens AG, 2017
            // To update group and header checkbox selection
            var areAllRowsSelected;
            angular.forEach($scope.renderedRows, function (group) {
                if (group.children && _.where(group.children, rowItem.orig).length > 0) {
                    var areAllRowsSelected = true;
                    for (var i = 0 ; i < group.children.length; i++) {
                        if (!group.children[i].entity.selected) {
                            areAllRowsSelected = false;
                            break;
                        }
                    }
                    group.entity.selected = areAllRowsSelected;
                }
            })
        ...
        self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
            ...
            if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
                ...
                // © Siemens AG, 2017
                // To update group and header checkbox selection
                var groupRows = grid.rowFactory.aggCache;
                for (var i = 0; i < groupRows.length ; i++) {
                    groupRows[i].entity.selected = checkAll;
                }
                $scope.page = {
    ...
    var ngStyleProvider = function ($scope, grid) {
        ...
        $scope.rowStyle = function (row) {
            // © Siemens AG, 2014
            var ret = { "top": row.offsetTop + "px", "height": $scope.rowHeight - 2 + "px" };
            if (row.isAggRow) {
                ...
                return {
                    pre: function ($scope, iElement, iAttrs) {
                        ...
                        return grid.init().then(function () {
                            ...
                            // © Siemens AG, 2015
                            options.expandGroup = function (field, label) {
                                var agg, i;
                                if (field) {
                                    grid.rowFactory.aggCache.some(function (val) {
                                        if (val && val.field === field && val.label === label) {
                                            agg = val;
                                            return true;
                                        }
                                        return false;
                            ...
                            options.sortBy = function (field, sortDirection) {
                                ...
                                if (col) {
                                    // © Siemens AG, 2015
                                    if (sortDirection) {
                                        // col.sort will toggle the sort direction.  Seems safer than changing the col.sort() function.
                                        col.sortDirection = (sortDirection === ASC) ? DESC : ASC;
                                    }
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('aggregateTemplate.html',
          // © Siemens AG, 2015
          "<div id=\"group-header-container\" ng-style=\"rowStyle(row)\" style=\"overflow:hidden;\"; class=\"ngAggregate\">\r" +
            "\n" +
            "<div ng-style=\"{height: rowHeight}\" class=\"uyAggregateText\" ng-show=\"multiSelect\">\r" +
            "\n" +
                "<div id=\"group-check-container\" style=\"display:table-cell; vertical-align:middle; height:inherit;\">\r" +
                    "<div style=\"line-height:1em\"><input type=\"checkbox\" style=\"margin: 0 10px 0 2px; cursor: pointer\" ng-click=\"selectAggregate(row.getAggregateSelectArg(true));\" ng-checked=\"row.getCheckState();\" /></div>\r" +
                "\n" +
                "</div>\r" +
            "\n" +
            "</div>\r" +
          "\n" +
          "    <div ng-show=\"row.showSvgIcon()\" style=\"display:inline-block; float:left;\"><div id=\"group-arrow-container\" sit-mom-icon=\"{'path':  'common/icons/miscRightArrow16.svg','size':'16px'}\" ng-class=\"'uyAggArrow'\" ng-click=\"row.toggleExpand()\" ng-style=\"{height: rowHeight}\"></div></div>\r" +
          "    <div ng-show=\"!row.showSvgIcon()\" style=\"display:inline-block; float:left;\"><div id=\"group-arrow-container\" sit-mom-icon=\"{'path':  'common/icons/miscDownArrow16.svg','size':'16px'}\" ng-class=\"'uyAggArrow'\" ng-click=\"row.toggleExpand()\" ng-style=\"{height: rowHeight}\"></div></div>\r" +
          "\n" +
              "<div ng-style=\"{height: rowHeight}\" class=\"uyAggregateText\" >\r" +
              "\n" +
                  "<div id=\"group-text-container\" ng-click=\"row.toggleExpand()\" style=\"display:table-cell; vertical-align:middle; height:inherit; padding-bottom:2px\">{{row.label CUSTOM_FILTERS}} ({{row.totalChildren()}}{{AggItemsLabel}})</div>\r" +
              "\n" +
              "</div>\r" +
              "\n" +
              "<div class=\"uyAggLineContainer\" ng-style=\"{height: rowHeight}\">\r" +
                  "\n" +
                      "<div id=\"group-line-container\" class=\"uyAggLine\"></div>\r" +
                  "\n" +
              "</div>\r" +
              "\n" +
          "</div>\r" +
        ...
        //  "<div id=\"group-header-container\" ng-style=\"rowStyle(row)\" style=\"overflow:hidden;\"; class=\"ngAggregate\">\r" +
        //  "\n" +
        //  "    <div style=\"display:inline-block; float:left;\" ng-click=\"row.toggleExpand()\" ><div id=\"group-arrow-container\" class=\"{{row.aggClass()}}\" ng-style=\"{height: rowHeight}\"></div></div>\r" +
        //  "\n" +
        //      "<div ng-style=\"{height: rowHeight}\" class=\"uyAggregateText\" ng-click=\"row.toggleExpand()\" >\r" +
        //      "\n" +
        //          "<div id=\"group-text-container\" style=\"display:table-cell; vertical-align:middle; height:inherit; padding-bottom:2px\">{{row.label CUSTOM_FILTERS}} ({{row.totalChildren()}}{{AggItemsLabel}})</div>\r" +
        //      "\n" +
        //      "</div>\r" +
        //      "\n" +
        //      "<div ng-style=\"{height: rowHeight}\" class=\"uyAggregateText\" ng-show=\"multiSelect\" ng-click=\"selectAggregate(row.getAggregateSelectArg(true));\">\r" +
        //      "\n" +
        //          "<div id=\"group-check-container\" style=\"display:table-cell; vertical-align:middle; height:inherit;\">\r" +
        //              "<div style=\"line-height:1em\"><input type=\"checkbox\" style=\"margin-left:10px; cursor: pointer\" ng-checked=\"row.getCheckState();\" /></div>\r" +
        //          "\n" +
        //          "</div>\r" +
        //      "\n" +
        //      "</div>\r" +
        ...
        $templateCache.put('cellTemplate.html',
          "<div id=\"data-cell-container\" class=\"ngCellText\" ng-class=\"col.colIndex()\"><span id=\"text-container\" ng-cell-text>{{COL_FIELD CUSTOM_FILTERS}}</span></div>"
        );
        ...
        $templateCache.put('checkboxHeaderTemplate.html',
          "<input class=\"ngSelectionHeader\" type=\"checkbox\" ng-show=\"multiSelect\" ng-model=\"page.allSelected\" ng-change=\"toggleSelectAll(page.allSelected, true)\"/>"
        );
        ...
        $templateCache.put('footerTemplate.html',
          "<div ng-show=\"showFooter\" class=\"ngFooterPanel\" ng-class=\"{'ui-widget-content': jqueryUITheme, 'ui-corner-bottom': jqueryUITheme}\" style=\"width:100%\" ng-style=\"footerStyle()\">\r" +
          "\n" +
          ...
          "\n" +
          // © Siemens AG, 2016
          "            <span class=\"ngLabel\">{{'pager.total-items' | translate}}: {{maxRows()}}</span><span ng-show=\"filterText.length > 0\" class=\"ngLabel\">({{'pager.filtered-items' | translate}}: {{totalFilteredItemsLength()}})</span>\r" +
          "\n" +
          "        </div>\r" +
          "\n" +
          "        <div class=\"ngFooterSelectedItems\" ng-show=\"multiSelect\">\r" +
          "\n" +
          "            <span class=\"ngLabel\">{{'pager.selected-items' | translate}}: {{selectedItems.length}}</span>\r" +
          "\n" +
          "        </div>\r" +
          "\n" +
          "    </div>\r" +
          ...
          "\n" +
          // © Siemens AG, 2016
          "            <span style=\"float: left; margin-top: 3px; margin-right: 1px;\" class=\"ngLabel\">{{'pager.page-size' | translate}}:</span>\r" +
          "\n" +
          // © Siemens AG, 2014
          "            <select style=\"float: left;height: 27px; width: 100px\" ng-model=\"pagingOptions.pageSize\" class=\"uyRowCountSelect\" ng-options=\"pageSize for pageSize in pagingOptions.pageSizes\" >\r" +
          "\n" +
          ...
          "\n" +
          // © Siemens AG, 2016
          "        <div style=\"float:left; margin-right: 10px;\" class=\"ngPagerControl\" style=\"float: left; min-width: 135px;\">\r" +
          "\n" +
          "            <button type=\"button\" class=\"ngPagerButton\" ng-click=\"pageToFirst()\" ng-disabled=\"cantPageBackward()\" title=\"{{'pager.page-first' | translate}}\"><div class=\"ngPagerFirstTriangle\"><div class=\"ngPagerFirstBar\"></div></div></button>\r" +
          "\n" +
          "            <button type=\"button\" class=\"ngPagerButton\" ng-click=\"pageBackward()\" ng-disabled=\"cantPageBackward()\" title=\"{{'pager.page-prev' | translate}}\"><div class=\"ngPagerFirstTriangle ngPagerPrevTriangle\"></div></button>\r" +
          "\n" +
          // © Siemens AG, 2014
          "            <div style=\"display:inline-block; vertical-align:top;\">" +
          "\n" +
          // © Siemens AG, 2016
          "                <input class=\"ngPagerCurrent\" min=\"1\" max=\"{{currentMaxPages}}\" type=\"number\" style=\"width:50px; height: 25px; padding: 0 4px;\" ng-model=\"pagingOptions.currentPage\"/>\r" +
          "\n" +
          "                <span class=\"ngGridMaxPagesNumber\" ng-show=\"maxPages() > 0\">/ {{maxPages()}}</span>\r" +
          "\n" +
          "            </div>" +
          "\n" +
          "            <button type=\"button\" class=\"ngPagerButton\" ng-click=\"pageForward()\" ng-disabled=\"cantPageForward()\" title=\"{{'pager.page-next' | translate}}\"><div class=\"ngPagerLastTriangle ngPagerNextTriangle\"></div></button>\r" +
          "\n" +
          "            <button type=\"button\" class=\"ngPagerButton\" ng-click=\"pageToLast()\" ng-disabled=\"cantPageToLast()\" title=\"{{'pager.page-last' | translate}}\"><div class=\"ngPagerLastTriangle\"><div class=\"ngPagerLastBar\"></div></div></button>\r" +
          "\n" +
          "        </div>\r" +
          "\n" +
          "    </div>\r" +
        ...
        $templateCache.put('gridTemplate.html',
            // © Siemens AG, 2014
          "<div id=\"header-container\" class=\"ngTopPanel\" ng-class=\"{'ui-widget-header':jqueryUITheme, 'ui-corner-top': jqueryUITheme}\" style=\"width:100%\" ng-style=\"topPanelStyle()\">\r" +
          "\n" +
          "    <div class=\"ngGroupPanel\" ng-show=\"showGroupPanel()\" style=\"width:100%\" ng-style=\"groupPanelStyle()\">\r" +
          "\n" +
          ...
          "    </div>\r" +
          "\n" +
          "    <div class=\"ngHeaderContainer\" style=\"width:100%\" ng-style=\"headerStyle()\">\r" +
          "\n" +
          ...
          "</div>\r" +
          "\n" +
          "<div id=\"data-container\" class=\"ngViewport\" unselectable=\"on\" ng-viewport ng-class=\"{'ui-widget-content': jqueryUITheme}\" style=\"width:100%\" ng-style=\"viewportStyle()\">\r" +
          "\n" +
          "    <div class=\"ngCanvas\" ng-style=\"canvasStyle()\">\r" +
          "\n" +
          "        <span id=\"no-data-message-container\" class=\"uyNoDataMessage\" ng-if=\"renderedRows.length===0\">{{noDataMessage}}</span>\r" +
          "        <div id=\"data-row-container\" ng-style=\"rowStyle(row)\" ng-repeat=\"row in renderedRows\" ng-click=\"row.toggleSelected($event)\" ng-class=\"row.alternatingRowClass()\" ng-row></div>\r" +
          "\n" +
          "    </div>\r" +
          "\n" +
          "</div>\r" +
          "\n" +
          "<div id=\"footer-container\" ng-grid-footer></div>\r" +
          "\n"
        );
        ...
        $templateCache.put('headerCellTemplate.html',
          ...
          "\n" +
          "    <div id=\"header-sort-button\" ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\">{{col.displayName}}</div>\r" +
          "\n" +
          ...
          "</div>\r" +
          "\n" +
          "<div id=\"header-resize-button\" ng-show=\"col.resizable\" class=\"ngHeaderGrip\" ng-click=\"col.gripClick($event)\" ng-mousedown=\"col.gripOnMouseDown($event)\"></div>\r" +
          "\n"
        );
        ...
        $templateCache.put('rowTemplate.html',
          ...
          "\n" +
           // © Siemens AG, 2014
           // setting the row height in ng-cell div.
          "\t<div ng-cell ng-style=\"{height: rowHeight}\"></div>\r" +
          "\n" +
          "</div>"
        );
